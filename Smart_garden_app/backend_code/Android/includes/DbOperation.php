<?php
    class DbOperator{
        ...
        function changeDeviceProperties($input_id, $new_input_name, $new_output_id, $new_output_name, $new_threshold){
            $stmt =$this->con->prepare("
            UPDATE devices SET input_name = ?, output_id = ?, output_name = ?, threshold = ? WHERE input_id = ?;
            ");
            $stmt->bind_param("sssss", $new_input_name, $new_output_id, $new_output_name, $new_threshold, $input_id);
            if($stmt->execute()){
                return true;
            }
            else{
        ...
        function changePlantSetting($user_id, $plant_name, $buy_date, $new_amount, $new_buy_location){
            if(!$this->checkPlantExist($user_id, $plant_name, $buy_date)){
                return 0;
            }
            $convert_user_id = (int)$user_id;
            $convert_amount = (int)$new_amount;
            $stmt =$this->con->prepare("UPDATE plant SET Buy_location = ?, Amount = ? 
            WHERE User_ID = ? AND Plant_name = ? AND Buy_date = ?");
            $stmt->bind_param("siiss", $new_buy_location, $convert_amount, $convert_user_id, $plant_name, $buy_date);
            if($stmt->execute()){
                return 1;
            }
            else{
